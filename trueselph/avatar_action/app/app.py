...
def render(router: StreamlitRouter, agent_id: str, action_id: str, info:dict):
    ...
    if st.session_state[model_key]['image_data']:
        st.image(decode_base64_image(st.session_state[model_key]['image_data']), caption='image preview', width=200)
    ...
    if selected_file is not None:
        ...
        if st.button('Upload'):
            ...
            if result := call_set_avatar(agent_id, module_root, files_list):
...
def call_set_avatar(agent_id: str, module_root:str, files:dict):
    return call_action_walker_exec(agent_id, module_root, 'set_avatar', None, files)  


def call_get_avatar(agent_id: str, module_root:str):
