...
def dummy(prog=None, msg=""):
    pass
@manager.route('/document/upload_and_chunk', methods=['POST'])  # noqa: F821
def upload_and_chunk():
    ...
    file_objs = request.files.getlist('file')
    for file_obj in file_objs:
        print(type(file_obj))  # Should print <class 'werkzeug.datastructures.FileStorage'>
        ...
        #     return get_json_result(
        #         data=False, message='No file selected!', code=settings.RetCode.ARGUMENT_ERROR)
        return chunk(file_obj, binary=True, from_page=0, to_page=100000,
