...
def render(router:StreamlitRouter, agent_id: str, action_id: str, info:dict):
    ...
    with st.expander('Import Knodes', False):
        ...
        knode_data = st.text_area('Agent Knodes in YAML or JSON', value="", height=170, key=f"{model_key}_knode_data")
        embeddings = st.toggle("Import with Embeddings", value=True, key=f"{model_key}_import_embeddings_json")

        if st.button('Import', key=f"{model_key}_btn_import_knodes"):
            # Call the function to import
            if result := call_action_walker_exec(agent_id, module_root, 'import_knodes', {'data': knode_data, 'embeddings': embeddings}):
                st.success("Agent knode imported successfully")
            else:
                st.error("Failed to import knodes. Ensure that the  descriptor is in valid YAML or JSON format.")
               
            uploaded_file = st.file_uploader("Upload file", key=f"{model_key}_agent_knode_upload")
            ...
            if uploaded_file is not None:
                ...
                if loaded_config:
                    ...
                    if imported:= call_action_walker_exec(agent_id, module_root, 'import_knodes', {'data': knode_data, 'embeddings': embeddings}):
                        st.success("Agent knodes imported successfully")
                    else:
                        st.error("Failed to import agent knode. Ensure that you are uploading a valid YAML file")
    ...
    with st.expander('Export Knodes', False):
        ...
        embeddings = st.toggle("Export with Embeddings", value=True, key=f"{model_key}_export_embeddings_json")
        ...
        if st.button('Export', key=f"{model_key}_btn_export_knodes"):
            ...
            result = call_action_walker_exec(agent_id, module_root, 'export_knodes', params)
    ...
    with st.expander('Purge Collection', False):
        ...
        if st.button('Delete all documents', key=f"{model_key}_btn_delete_collection"):
            ...
            if result := call_action_walker_exec(agent_id, module_root, 'delete_collection'):
    ...
    if result:
        ...
        render_paginated_documents(model_key, agent_id, module_root, documents, page, per_page, total)
    else: 
        st.error("Unable to list documents")
            
    
def render_paginated_documents(model_key, agent_id, module_root, documents, page, per_page, total):
    st.divider()
    st.subheader('Documents')
    
    for document in documents:
        text_preview = document['text'][:200] + ('...' if len(document['text']) > 100 else '')
        ...
        if st.session_state.get(f'show_edit_document_form_{document['id']}', False):
            with st.form(f'edit_document_form_{document['id']}'):
                edit_document_text = st.text_area("Document Text", value=document['text'])
                submitted = st.form_submit_button("Update")
                
                if submitted:
                    # Call the function to add the new text document
                    if call_update_document(agent_id, module_root, id=document['id'], data={'text': edit_document_text}):
                        st.session_state[f'show_edit_document_form_{document['id']}'] = False
                        st.rerun()
                    else:
...
def call_list_documents(agent_id: str, module_root:str, page:int, per_page:int):

    args = {"page": page, "per_page": per_page}
    return call_action_walker_exec(agent_id, module_root, 'list_documents', args)
    
def call_add_texts(agent_id: str, module_root:str, texts:list[str]):
    
    args = {"texts": texts}
    return call_action_walker_exec(agent_id, module_root, 'add_texts', args)

def call_delete_document(agent_id: str, module_root:str, id:str):
    
    args = {"id": id}
    return call_action_walker_exec(agent_id, module_root, 'delete_document', args)

def call_update_document(agent_id: str, module_root:str, id:str, data: dict):
